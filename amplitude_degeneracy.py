...
"""
The ampltitude degeneracy only occurs when the total amplitude of the 
B vector at a position is determined by only free parameters. 
"""
...
def run():
    ...
    print("B_arm_2: ", Bfield.parameters['b_arm_2'])
    field_list = [Bfield, cre]
    simulation1 = test_sim(field_list)
    brightness_normal = simulation1[key].data[0] * simulation1[key].unit
    
    #Bfield.parameters['b_arm_2'] = -100
    #print("B_arm_2: ", Bfield.parameters['b_arm_2'])
    #field_list = [Bfield, cre] 
    #simulation2 = test_sim(field_list)
    #brightness_flipped = simulation2[key].data[0] * simulation2[key].unit
    
    print(brightness_normal[:10])
    #print(brightness_flipped[:10])
