...
def apply_erf(wgt, dist, lo, mid, hi, sig, erf):
    ...
    wgt[mask] = 0.
    mask = (dist > lo) & (dist <= hi)
    wgt[mask] *= erf((dist[mask]-mid)/sig)
    return wgt
...
class LOSResponse(LinearOperator):
    ...
    THIS IS A CUSTOM VERSION
    Changes in this script are made to enable the use of gaussian distributed
    distance errors, not gaussian parallax errors.
    ...
    def __init__(self, domain, starts, ends, sigmas=None, truncation=3.):
        ...
        # THIS IS A CUSTOM VERSION
        real_distances = difflen + truncation*sigmas
